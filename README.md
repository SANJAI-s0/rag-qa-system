...
[![HuggingFace](https://img.shields.io/badge/🤗_HF-embeddings-yellow)](https://huggingface.co/)
...
[![Repo Size](https://img.shields.io/github/repo-size/SANJAI-s0/rag-qa-system)]()
...
│
├── data/                            # All data used by the system
│   │
│   ├── raw/                         # Original research papers (PDFs)
│   │   ├── 1706.03762v7.pdf         # "Attention Is All You Need" (Transformer)
│   │   ├── 2005.11401v4.pdf         # Retrieval-Augmented Generation (RAG) paper
│   │   └── 2005.14165v4.pdf         # GPT-3 paper
│   │
│   └── processed/                   # Preprocessed document chunks
│       ├── chunks/                  # Individual text chunks (optional storage)
│       └── chunks.pkl               # Serialized chunks used for embedding
│
├── models/                          # Saved ML artifacts
│   │
│   └── faiss_index/                 # FAISS vector database for dense retrieval
│       ├── index.faiss              # Binary FAISS index storing embeddings
│       └── index.pkl                # Metadata mapping chunks to source documents
│
├── notebooks/                       # Jupyter notebooks for experimentation
│   │
│   ├── 01_exploratory_analysis.ipynb    # Inspect PDFs, extracted text, chunk sizes
│   ├── 02_retrieval_testing.ipynb       # Test similarity search and retrieval quality
│   └── 03_results_visualization.ipynb   # Visualize evaluation metrics and results
│
├── outputs/                         # Outputs generated by the pipeline
│   │
│   ├── answers/                     # Final generated answers for each query
│   │   ├── answer_1.txt
│   │   ├── answer_2.txt
│   │   ├── answer_3.txt
│   │   ├── answer_4.txt
...
│   │   └── pipeline.log             # Execution log of the full pipeline
│   │
│   └── results/                     # Evaluation outputs
│       ├── results_YYYYMMDD_HHMMSS.json  # Detailed evaluation metrics
│       └── evaluation_plots.png         # Visualization of evaluation results
│
...
│   │   │
│   │   ├── __init__.py
│   │   ├── document_loader.py       # Loads PDFs and extracts page-level text
│   │   ├── text_splitter.py         # Splits documents into overlapping chunks
│   │   └── embedding_generator.py   # Converts chunks into vector embeddings
│   │
│   ├── phase2_retrieval/            # Phase 2: Information retrieval
│   │   │
│   │   ├── __init__.py
│   │   ├── vector_store.py          # Builds, saves, and loads FAISS vector index
│   │   ├── retriever.py             # Retrieves top-K relevant chunks
│   │   └── hybrid_search.py         # Optional BM25 + dense retrieval hybrid search
│   │
│   ├── phase3_generation/           # Phase 3: LLM answer generation
│   │   │
│   │   ├── __init__.py
│   │   ├── llm_integration.py       # Handles communication with Google Gemini API
│   │   ├── prompt_templates.py      # Prompt templates for RAG queries
│   │   └── answer_formatter.py      # Formats answers with sources & citations
│   │
│   ├── phase4_evaluation/           # Phase 4: Evaluation and metrics
│   │   │
│   │   ├── __init__.py
│   │   ├── metrics.py               # Calculates retrieval & generation metrics
│   │   ├── results_analyzer.py      # Aggregates and visualizes results
│   │   └── test_queries.py          # Predefined evaluation queries
│   │
...
│       └── visualization.py         # Plotting utilities for evaluation graphs
│
├── tests/                           # Unit tests for system components
│   │
│   ├── test_preprocessing.py        # Tests document loading and chunking
│   ├── test_retrieval.py            # Tests FAISS retrieval accuracy
│   └── test_generation.py           # Tests LLM answer generation pipeline
│
├── scripts/                         # Utility scripts for automation
│   │
│   ├── download_papers.py           # Downloads the required research papers
│   ├── download_papers.sh           # Linux/Mac wrapper for downloading papers
│   └── download_papers.bat          # Windows wrapper for downloading papers
│
...
├── cleanup.bat                      # Windows script to delete FAISS index files
├── docker-compose.yml               # Docker Compose service configuration
├── Dockerfile                       # Docker image definition
├── fix_init_files.py                # Auto-creates missing __init__.py files
│
├── README.md                        # Project documentation
├── requirements.txt                 # Python dependencies
|
├── download_papers.py               # Script to download the three research PDFs into data/raw/
├── download_papers.sh               # Linux/Mac wrapper for download_papers.py
├── download_papers.bat              # Windows wrapper for download_papers.py
│
├── run_pipeline.py                  # Main script that executes the entire RAG pipeline
└── test_gemini_working.py           # Verifies Gemini API connectivity
```
...
Place the required research papers in the `data/raw/` directory. You can download them using the links below or run the provided scripts to automatically fetch them. Alternatively, you may download the files manually and place them in the `data/raw/` folder.
